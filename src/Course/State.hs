...
instance Functor (State s) where
  ...
  (<$>) f fa = State (\s -> let (a, s') = runState fa s in (f a, s'))
...
instance Applicative (State s) where
  ...
  (<*>) fab fa = State (\s -> let (f, s') = runState fab s in let (a, s'') = runState fa s' in (f a, s''))
...
instance Monad (State s) where
  ...
  (=<<) f fa = State (\s -> let (a, s') = runState fa s in runState (f a) s')
